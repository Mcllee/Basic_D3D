...
void CTerrainMesh::ReleaseUploadBuffers()
{
...
void CTerrainMesh::Render(ID3D12GraphicsCommandList* pd3dCommandList)
{
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList,
	...
	XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void* pContext) : CTerrainMesh(pd3dDevice, pd3dCommandList)
{
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++)
	{
		for (int x = xStart; x < (xStart + nWidth); x++, i++)
		{
			...
			XMFLOAT4 xmf3Color;
			if (OnGetHeight(x, z, pContext) < 5.5f) {
				xmf3Color = Vector4::Add(OnGetColor(x, z, pContext), XMFLOAT4(0, 0.2, 0.5, 1));
			}
			else if (OnGetHeight(x, z, pContext) >= 5.5f && OnGetHeight(x, z, pContext) < 250.5f) {
				xmf3Color = Vector4::Add(OnGetColor(x, z, pContext), XMFLOAT4(0.4, 0.2, 0.2, 1));
			}
			else if (OnGetHeight(x, z, pContext) >= 250.5f && OnGetHeight(x, z, pContext) <= 500.5f) {
				xmf3Color = Vector4::Add(OnGetColor(x, z, pContext), XMFLOAT4(0.2, 0.1, 0.1, 1));
			}
			else {
				xmf3Color = Vector4::Add(OnGetColor(x, z, pContext), xmf4Color);
			}
